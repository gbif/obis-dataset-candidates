...
# Search GBIF datasets by keyword using rgbif::dataset_search with pagination
search_gbif_datasets <- function(keyword, limit_per_page = 100) {
  all_results <- list()
  offset <- 0
  ...
  tryCatch({
    repeat {
      ...
      # dataset_search returns a list with $data component
      if (!is.null(result$data) && nrow(result$data) > 0) {
        all_results <- append(all_results, list(result$data))
        
        # Check if we've reached the end
        if (nrow(result$data) < limit_per_page) break
        if (result$meta$endOfRecords) break
    ...
    if (length(all_results) > 0) {
      ...
      if ("datasetKey" %in% names(combined)) {
        combined <- combined %>% rename(key = datasetKey)
      }
      return(combined)
    } else {
      return(NULL)
...
find_candidates <- function() {
  ...
  for (keyword in marine_keywords) {
    ...
    # dataset_search returns paginated results
