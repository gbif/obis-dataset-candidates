...
percentage_worms <- read_tsv("percentage_worms.tsv", show_col_types = FALSE)

# Read the GBIF candidates file
gbif_candidates <- read_tsv("gbif_candidates.tsv", show_col_types = FALSE)
...
filtered_candidates <- gbif_candidates %>%
  ...
  filter(worms_percentage >= 60) %>%
  # Reorder columns to show WORMS stats prominently
  select(key, title = title.x, publishingOrganizationTitle, matched_keywords,
...
write_tsv(filtered_candidates, "gbif_candidates_filtered.tsv")

cat("\nFiltered candidates saved to: gbif_candidates_filtered.tsv\n")
