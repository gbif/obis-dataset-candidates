...
library(jsonlite)  # For fetching OBIS organizations from API

# Load marine keywords from OBIS word frequency analysis
load_marine_keywords <- function(freq_file = "../../data/obis_word_frequencies.csv", 
                                  top_n = 100) {
...
# Fetch OBIS network organizations using API
fetch_obis_organizations <- function() {
  ...
  if (length(all_orgs) > 0) {
    ...
    cat(sprintf("Fetched %d OBIS network organizations\n", nrow(orgs_df)))
    return(orgs_df$key)
  } else {
    return(character(0))
  }
...
find_candidates <- function() {
  ...
  obis_org_keys <- fetch_obis_organizations()
  obis_network_uuid <- "2b7c7b4f-4d4f-40d3-94de-c28b6fa054a6"
  
  cat("OBIS datasets to exclude:", length(obis_keys), "\n")
  cat("OBIS organizations to exclude:", length(obis_org_keys), "\n")
  ...
  for (keyword in marine_keywords) {
    ...
    if (!is.null(result) && is.data.frame(result) && nrow(result) > 0) {
      # Filter out datasets already in OBIS using three criteria:
...
track_matched_keywords <- function(candidates_df, marine_keywords) {
  ...
  candidates_df <- candidates_df %>%
    rowwise() %>%
    mutate(
      # Use title for matching (description not available in dataset_export)
      text = tolower(title %||% ""),
      matched_keywords = {
    ...
    ungroup() %>%
    select(key, title, publishingOrganizationKey, publishingOrganizationTitle, matched_keywords)
...
# Find candidates using dataset_export (returns all matches)
...
if (nrow(candidates) > 0) {
  ...
  # Save results as TSV
  output_file <- "../../data/gbif_candidates.tsv"
